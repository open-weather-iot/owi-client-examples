...
```{r}
colnames(weather_data_series_json$raw)
```

Vamos selecionar apenas as colunas que contém o valor e unidade de medida da temperatura da CPU do sistema:
```{r}
print(weather_data_series_json$raw$'SYS/cpu/temperature'[c('value', 'unit')])
```

A média da temperatura da CPU do sistema é:
```{r}
mean(weather_data_series_json$raw$'SYS/cpu/temperature'$value)
```
